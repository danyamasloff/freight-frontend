...
export default function FleetPage() {
    ...
    const stats = {
        ...
        outOfService: vehicles?.filter(v => v.status === VehicleStatus.OUT_OF_SERVICE).length || 0,
    };

    const utilizationRate = stats.total > 0 ? Math.round((stats.inUse / stats.total) * 100) : 0;
    const availabilityRate = stats.total > 0 ? Math.round(((stats.available + stats.inUse) / stats.total) * 100) : 0;
                ...
                {isLoading ? (
                    ...
                    <Card className="border shadow-sm">
                        <CardContent className="text-center py-12">
                            ...
                            <h3 className="text-lg font-medium mb-2">Транспортные средства не найдены</h3>
                                        ...
                                        <CardContent className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 text-sm">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="font-medium">
                                                        {new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toLocaleDateString('ru-RU')}
                                                    </div>
                                                </div>
                                            </div>
